...
describe('createPipe', () => {
  describe('#01 => entry-only pipeline', () => {
    ...
    test('#02 => entry with multiple params', () => {
      const fn = createPipe((x: number, name: string) => ({
        value: x,
        name,
      }));
  ...
  describe('#06 => assign helper', () => {
    ...
    test('#02 => assign inside pipeline action', () => {
      const fn = createPipe((x: number) => ({ value: x }), 'bump').define({
        actions: {
  ...
  describe('#07 => Describer objects (name + description)', () => {
    test('#01 => using a function as first argument, keeping the test flow', () => {
      const fn = createPipe((x: number) => ({ value: x }));
